    ...
    if feed.has_next():
        ...
        for tkr, hist in tick_batch.items():
            ...
            if sig.action in ("buy","sell"):
                ...
                # sizing
                qty = risk.size_fractional(equity=broker.cash + sum(pos.qty*pos.avg_price for pos in broker.positions.values()), price=price)
                # constrain to cash and fractional
                if qty <= 0:
                    continue
                reason_json = json.dumps(sig.reason)
                broker.execute_market(ts=tick_ts, ticker=tkr, side=sig.action, qty=qty, price=price,
...
with col2:
    ...
    for t, p in broker.positions.items():
        pos_rows.append({"ticker": t, "qty": p.qty, "avg_price": p.avg_price})
